...
export const StatusBadge: React.FC<StatusBadgeProps> = ({ status, isTerminating, className }) => {
  if (isTerminating) {
    return (
      <span
        className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300 animate-pulse border border-gray-300 dark:border-gray-600 ${className || ''}`}
      >
        Terminating
      </span>
    );
  ...
  return (
    <span
      className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold transition-colors ${colorClasses} ${className || ''}`}
    >
      {status || 'Unknown'}
    </span>
  );
