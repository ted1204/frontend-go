...
export const getUsersByGroup = async (g_id: number): Promise<UserGroupUser[]> => {
  try {
    ...
    if (!dataMap || typeof dataMap !== 'object') {
      throw new Error('getUsersByGroup: Expected object response but received invalid data');
    }
    ...
    throw new Error(
      `Fetch users by group error: ${error instanceof Error ? error.message : 'Unknown error'}`,
    );
  }
};
...
export const getGroupsByUser = async (u_id: number): Promise<UserGroupGroup[]> => {
  try {
    ...
    if (!data || !Array.isArray(data)) {
      throw new Error('getGroupsByUser: Expected array response but received invalid data');
    }
